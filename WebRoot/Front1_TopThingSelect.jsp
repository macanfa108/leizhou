...
<script type="text/javascript">
	function deleteForm(id) {
		if (confirm("确定要删除此精品吗？")) {
			window.location.href = "ThingServlet?method=3&id=" + id;
		}
	}
</script>
...
--%>
				<div class="box-body">
					<form align="right" class="form-search s-widget" name="form1"
						method="post" action="backstage_ThingSelect.jsp"
						onSubmit="return userCheck()">
						<span style="width:100px;height:50px;bgcolor:yellow;float:left;"
							font-size:16pt><h1>精品列表</h1></span>
						<div class="input-append">

							<input type="text" id="inputPassword" placeholder="请输入精品名"
								style="height:31px;width:200px" name="thingName">
					...
					<table class='table table-hover table-responsive'>
						<tr>
							<td width="200">精品类型</td>
							<td width="200">精品名</td>
							<td width="200">精品详情</td>
						...
						<%
							for (int i = 0; i < list.size(); i++) {
								ThingBean thingForm = (ThingBean) list.get(i);

								String content = thingForm.getThingContent();
						...
						%>
						<tr>
							<td><%=thingForm.getThingName()%></td>
							<td><%=thingForm.getTypeName()%></td>
							<td><%=content%></td>
							<td><%=thingForm.getThingTime()%></td>
							<td><%=thingForm.getLikeNum()%></td>
							<!--   
             <td><div align="center" bgcolor="#009393"><a href="javascript:deleteForm('<%=thingForm.getThingId()%>')" title="可以查看相应的公告内容">置顶</a>&nbsp;&nbsp;<a href="javascript:deleteForm('<%=thingForm.getThingId()%>')">删除</a></div></td>
           -->
							<td><a
							class='btn btn-info'
								href="backstage_TopThingSelect.jsp?id=<%=thingForm.getThingId()%>"
								title="可以查看相应的公告内容">置顶</a>&nbsp;&nbsp;<a
								class='btn btn-danger'
								href="javascript:deleteForm('<%=thingForm.getThingId()%>')">删除</a>
							</td>
						</tr>
						<%
							}
