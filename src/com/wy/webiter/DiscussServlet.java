...
public class DiscussServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (method == 2) {
			this.updateDisuss(request, response);//	后台， 置顶公告内容
		}
		if (method == 3) {
			this.topDisuss(request, response);//	后台， 置顶公告内容
		}
...
//	后台， 修改评论内容
	public void updateDisuss(HttpServletRequest request,
		...
		if (disussDao.operationDiscuss("修改", disussForm)) {
			out
					.print("<script language=javascript>alert('修改公告信息成功！');window.location.href='backstage_DiscussSelect.jsp';</script>");
		} else {
			out
	...
	public void deleteDisuss(HttpServletRequest request,
		...
		if (disussDao.operationDiscuss("删除", disussForm)) {
			out
					.print("<script language=javascript>alert('删除公告信息成功！');window.location.href='backstage_DiscussSelect.jsp';</script>");
		} else {
			out
