...
*/
	...
	public void headAddNumberArticle(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		NewsBean newsForm = new NewsBean();
		newsDao = new NewsDao();
		newsForm.setNewsId(Integer.valueOf(request.getParameter("id")));
		newsDao.operationNews("增加", newsForm);
		request.setAttribute("form", newsDao.queryNewsForm(Integer
...
/*
	...
	*/
	...
	public void deleteArticle(HttpServletRequest request,
		...
		newsForm.setNewsId(Integer.valueOf(request.getParameter("id")));
		newsDao = new NewsDao();
		if (newsDao.operationNews("删除", newsForm)) {
			out
					.print("<script language=javascript>alert('删除新闻成功，请重新查询！');window.location.href='backstage_NewsSelect.jsp';</script>");
		} else {
			out
	...
	public void addArticle(HttpServletRequest request,
		...
		*/
		...
		Integer maxID = 1;
		if (newsDao.MaxQueryID() != null) {
			maxID = newsDao.MaxQueryID();
		}
				...
				for (int ii = 0; ii < imageType.length; ii++) {
					if (imageType[ii].equals(type)) {
						if (!singlefile.isMissing()) { // 如果选择了文件
							...
							System.out.println(filedir);
							newsForm.setNewsTitle(su.getRequest().getParameter("newsTitle"));
							newsForm.setNewsContent(su.getRequest().getParameter("newsContent"));
							newsForm.setNewsTime(su.getRequest().getParameter("newsTime"));
							newsForm.setPhotoURL(filedir);
							newsForm.setReadNum(0);
							
							if (newsDao.operationNews("添加", newsForm)) {
		...
		//request.setAttribute("result", result);
		RequestDispatcher requestDispatcher = request
				.getRequestDispatcher("backstage_NewsSelect.jsp");
		requestDispatcher.forward(request, response);
	}
				...
				for (int ii = 0; ii < imageType.length; ii++) {
					if (imageType[ii].equals(type)) {
						if (!singlefile.isMissing()) { 
							...
							filedir = filedir + maxID + "."
									+ singlefile.getFileExt();
							String s1 = Chinese.toChinese(su.getRequest().getParameter("typename"));
